					...
					switch ($value[1]) {
						...
						if ($value[1] === INT32)	{$tail = "0x01"; $fixed = 10;}
							...
							if ($value[0] === REP) {
								...
								$tmp = '';
								foreach ($arr as $ele) {
									$valuestr .= $keystr . $this->varintEncode($ele, $fixed, $tail);
								}
							} else {
						...
						case SINT32:case SINT64:
							...
							if ($value[0] === REP) {
								$arr = $this->$name;
								foreach ($arr as $ele) {
									$valuestr .= $keystr . $this->zigzagEncode($ele);
								}
							} else {
						...
						case FLOAT:case DOUBLE:
							...
							if ($value[0] === REP) {
								$arr = $this->$name;
								foreach ($arr as $ele) {
									$valuestr .= $keystr . $this->floatEncode($ele, $fixed);
								}
							} else {
						...
						case FIXED32:case FIXED64: case SFIXED32:case SFIXED64:
							...
							if ($value[0] === REP) {
								$arr = $this->$name;
								foreach ($arr as $ele) {
									$valuestr .= $keystr . $this->fixedEncode($ele, $fixed);
								}
							} else {
						...
						case STRING:case BYTES:
							...
							if ($value[0] === REP) {
								$arr = $this->$name;
								foreach ($arr as $ele) {
									$valuestr .= $keystr . $this->strEncode($ele);
								}
							} else {
